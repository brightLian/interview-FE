...
3. 将新创建的对象作为 this 的上下文（构造函数内部的 this 被赋值为这个新对象）
4. 返回新的对象
...
ES5：最常使用原型链 + 构造函数结合实现继承，这种方式称为组合寄生继承。（使用原型链实现对属性和方法的继承，使用构造函数来实现对实例属性的继承。）
...
### 下面 class 的四个属性分别属于 class 的什么，fn 和 f 有什么区别?
